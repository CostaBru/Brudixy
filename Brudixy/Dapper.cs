/*
 LicenseManager: http://www.apache.org/licenses/LICENSE-2.0 
...
this IDbConnection cnn, string sql, dynamic param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null
      ...
      /// Return a list of dynamic objects, reader is closed after the call
      /// </summary>
      public static IEnumerable<IDictionary<string, object>> Query(this IDbConnection cnn, string sql, object param, IDbTransaction transaction = null)
      {
         return Query<DapperRow>(cnn, sql, param, transaction);
      }
#else
        /// <summary>
        /// Return a list of dynamic objects, reader is closed after the call
...
this IDbConnection cnn, string sql, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, int? commandTimeout = null, CommandType? commandType = null
...
)
      {
         ...
         var data = QueryInternal<T>(cnn, sql, param as object, transaction, commandTimeout, commandType, null, null);
      ...
      /// created per row, and a direct column-name===member-name mapping is assumed (case insensitive).
      /// </returns>
      public static System.Data.DataTable QueryToDataTable<T>(

         this IDbConnection cnn, IDbCommand command
         )
      {
         var _dataTable = new System.Data.DataTable();

         loadAllToDataTable(cnn, command, _dataTable);

         return _dataTable;
      }

      private static void loadAllToDataTable(IDbConnection cnn, IDbCommand command, System.Data.DataTable _dataTable)
      {
         var _metaData = new DataTableMetaData();

         Action<IDataReader> _fillMetaData = r => fillMeta(r, _metaData);

         var _dapperRows = QueryInternalDapperRow<DapperRow>(cnn, _fillMetaData, command);
         ...
         foreach (var _dapperRow in _dapperRows)
         {
            if (_isFirst)
               prepareColumns(_dataTable, _metaData);

            _dataTable.LoadDataRow(_dapperRow.values, true);
      ...
      private static void prepareColumns(System.Data.DataTable _dataTable, DataTableMetaData _metaData)
      {
         var _columns = _dataTable.Columns;

         var _fieldNames = _metaData.Fields;
         var _types = _metaData.Types;

         for (int i = 0; i < _fieldNames.Length; i++)
         {
            _columns.Add(_fieldNames[i], _types[i]);
         }
      }

      private static void fillMeta(IDataReader r, DataTableMetaData metaData)
      {
         var _fieldCount = r.FieldCount;

         metaData.Fields = new string[_fieldCount];
         metaData.Types = new Type[_fieldCount];

         for (int i = 0; i < _fieldCount; i++)
         {
            var _name = r.GetName(i);
            var _type = r.GetFieldType(i);

            metaData.Fields[i] = _name;
            metaData.Types[i] = _type;
         }
      }

      private class DataTableMetaData
      {
         public string[] Fields;
         public Type[] Types;
      }

       private class CoreDataTableMetaData
       {
           public string[] Fields;

           public TableStorageType[] Types;

           public TableStorageTypeModifier[] TypeModifiers;
       }

       public static void FillDataTable<T>(

         this IDbConnection cnn, System.Data.DataTable toFill, IDbCommand command)
      {
         if (toFill.Columns.Count == 0)
         {
            loadAllToDataTable(cnn, command, toFill);
         }
         else
         {
            fillDataTable(cnn, toFill, command);
         }
      }

      private static void fillDataTable(IDbConnection cnn, System.Data.DataTable toFill, IDbCommand command)
      {
         var _metaData = new DataTableMetaData();
         ...
         var _sourceColumns = toFill.Columns;

         toFill.BeginLoadData();

         var _fieldNameToIndex = _sourceColumns.OfType<System.Data.DataColumn>()
                                               .ToDictionary(c => c.ColumnName.ToLower(), _sourceColumns.IndexOf);
         ...
         foreach (var _dapperRow in _dapperRows)
         {
            if (_isFirst)
            {
               _arraySize = prepareColumnsAndGetCount(_metaData, _sourceColumns, _fieldNameToIndex);
            }

            var _rowToLoad = new object[_arraySize];

            for (int i = 0; i < _metaData.Fields.Length; i++)
            {
               var _fieldName = _metaData.Fields[i];

               if (_fieldNameToIndex.TryGetValue(_fieldName.ToLower(), out var index))
               {
                  _rowToLoad[index] = _dapperRow.values[i];
               }
            }

            toFill.LoadDataRow(_rowToLoad, true);
         ...
         if (_isFirst)
         {
            prepareColumnsAndGetCount(_metaData, _sourceColumns, _fieldNameToIndex);
         }

         toFill.EndLoadData();

         _fieldNameToIndex.Clear();
         _fieldNameToIndex.Clear();
      }

      private static int prepareColumnsAndGetCount(DataTableMetaData _metaData, DataColumnCollection _sourceColumns,
                                                   Dictionary<string, int> _fieldNameToIndex)
      {
         var _fieldNames = _metaData.Fields;
         var _types = _metaData.Types;

         var _count = _sourceColumns.Count;

         for (int i = 0; i < _fieldNames.Length; i++)
         {
            var _fieldName = _fieldNames[i];

            if (_fieldNameToIndex.TryGetValue(_fieldName.ToLower(), out var index) == false)
            {
               _fieldNameToIndex.Add(_fieldName, _count);

               _sourceColumns.Add(_fieldName, _types[i]);
        ...
        private static void fillMeta(IDataReader r, CoreDataTableMetaData metaData)
        {
            var fieldCount = r.FieldCount;

            metaData.Fields = new string[fieldCount];
            metaData.Types = new TableStorageType[fieldCount];
            metaData.TypeModifiers = new TableStorageTypeModifier[fieldCount];
            ...
            for (int i = 0; i < fieldCount; i++)
            {
                ...
                var metaDataType = DataTable.GetColumnType(type);

                metaData.Fields[i] = field;
                metaData.Types[i] = metaDataType.type;
                metaData.TypeModifiers[i] = metaDataType.typeModifier;
            }
        }
       ...
       /// created per row, and a direct column-name===member-name mapping is assumed (case insensitive).
       /// </returns>
       public static DataTable QueryToHybridDataTable<T>(this IDbConnection cnn, IDbCommand command)
       {
           var dataTable = new DataTable();

           loadAllToDataTable(cnn, command, dataTable);

           return dataTable;
       }

       private static void loadAllToDataTable(IDbConnection cnn, IDbCommand command, CoreDataTable dataTable)
       {
           var metaData = new CoreDataTableMetaData();

           Action<IDataReader> fillMetaData = r => fillMeta(r, metaData);

           var dapperRows = QueryInternalDapperRow<DapperRow>(cnn, fillMetaData, command).ToData();
           ...
           try
           {
              ...
              var count = dapperRows.Count;

              dataTable.Capacity = Math.Max(count, dataTable.Capacity);

              var isFirst = true;

              for (int index = 0; index < count; index++)
              {
                 var dapperRow = dapperRows[index];
                 if (isFirst)
                 {
                    prepareColumns(dataTable, metaData);
                 }

                 var dataRow = dataTable.ImportRow(RowState.Unchanged, dapperRow.values);
       ...
       private static void prepareColumns(CoreDataTable dataTable, CoreDataTableMetaData metaData)
        {
            var fieldNames = metaData.Fields;
            var types = metaData.Types;

            for (int i = 0; i < fieldNames.Length; i++)
            {
                dataTable.AddColumn(fieldNames[i], types[i]);
            }
        }

        public static void FillDataTable<T>(

           this IDbConnection cnn, CoreDataTable toFill, IDbCommand command)
        {
            if (toFill.ColumnCount == 0)
            {
                loadAllToDataTable(cnn, command, toFill);
            }
            else
       ...
       private static void fillDataTable(IDbConnection cnn, CoreDataTable toFill, IDbCommand command)
       {
           var metaData = new CoreDataTableMetaData();
           ...
           toFill.Capacity = Math.Max(count, toFill.Capacity);

           var loadState = toFill.BeginLoad();

           try
           {
              var fieldNameToIndex = toFill.ColumnMapping;

              var arraySize = 0;

              var isFirst = true;

              object[] rowToLoad = null;

              for (int j = 0; j < count; j++)
              {
                 var dapperRow = dapperRows[j];
                 if (isFirst)
                 {
                    prepareColumns(metaData, toFill);

                    arraySize = toFill.ColumnCount;

                    rowToLoad = ArrayPool<object>.Shared.Rent(arraySize);

                    Array.Clear(rowToLoad, 0, arraySize);
                 }

                 for (int i = 0; i < metaData.Fields.Length; i++)
                 {
                    var fieldName = metaData.Fields[i];

                    if (fieldNameToIndex.TryGetValue(fieldName, out var column))
                    {
                       rowToLoad[column.ColumnHandle] = dapperRow.values[i];
                    }
                 }

                 toFill.ImportRowDirty(RowState.Unchanged, rowToLoad);

                 isFirst = false;

                 ArrayPool<object>.Shared.Return(rowToLoad, true);
              }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void prepareColumns(CoreDataTableMetaData metaData, CoreDataTable table)
        {
            var fieldNames = metaData.Fields;
            var dataTypes = metaData.Types;

            for (int i = 0; i < fieldNames.Length; i++)
            {
                var fieldName = fieldNames[i];
               
                if (table.ContainsColumn(fieldName) == false)
                {
                    table.AddColumn(fieldName, dataTypes[i]);
                }
            }
        }
...
this IDbConnection cnn, string sql, dynamic param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null
      ...
      /// </summary>
      internal static IEnumerable<T> QueryInternalDapperRow<T>(this IDbConnection cnn, Action<IDataReader> fillMetaData, IDbCommand command)
      {
         IDbCommand cmd = null;
         IDataReader reader = null;

         bool _wasClosed = cnn.State == ConnectionState.Closed;
         try
         {
            cmd = command;
            ...
            reader = cmd.ExecuteReader(_wasClosed ? CommandBehavior.CloseConnection : CommandBehavior.Default);
            _wasClosed = false; // *if* the connection was closed and we got this far, then we now have a reader
            // with the CloseConnection flag, so the reader will deal with the connection; we
            // still need something in the "finally" to ensure that broken SQL still results
            // in the connection closing itself
            ...
            var _deserializer = GetDapperRowDeserializer(reader, 0, -1, false);

            while (reader.Read())
            {
               yield return (T)_deserializer(reader);
            }
      ...
      /// Return a typed list of objects, reader is closed after the call
      /// </summary>
      internal static IEnumerable<T> QueryInternal<T>(this IDbConnection cnn, string sql, object param, IDbTransaction transaction, int? commandTimeout, CommandType? commandType, Func<IDataReader,int> getColumnHash,IDbCommand command)
      {
         ...
         try
         {
            cmd = command ?? SetupCommand(cnn, transaction, sql, info.ParamReader, param, commandTimeout, commandType);
            ...
            // still need something in the "finally" to ensure that broken SQL still results
            // in the connection closing itself
            var _deserializer = info.Deserializer;

            var _columnHashFunc = getColumnHash ?? GetColumnHash;

            int _columnHash = _columnHashFunc(reader);

            if (_deserializer.Func == null || _deserializer.Hash != _columnHash)
            {
               _deserializer = info.Deserializer = new DeserializerState(_columnHash, GetDeserializer(typeof(T), reader, 0, -1, false));
               SetQueryCache(identity, info);
            }

            var func = _deserializer.Func;
...
this IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, string splitOn = "Id", int? commandTimeout = null, CommandType? commandType = null
...
#endif
               else if (typeof(IDataParameterCollection).IsAssignableFrom(identity.parametersType))
               {
                  info.ParamReader = (cmd, obj) =>
                     {
                        var _parameterCollection = (IDataParameterCollection) obj;

                        foreach (IDataParameter _parameter in _parameterCollection)
                        {
                           var _dataParameter = _parameter;

                           var _cloneable = _dataParameter as ICloneable;

                           if (_cloneable != null)
                           {
                              _dataParameter = (IDataParameter) _cloneable.Clone();
                           }

                           cmd.Parameters.Add(_dataParameter);
                        }
                     };
               }
               else
          ...
          , IDictionary<string, object>
      {
         ...
         {
            get {
               TryGetValue(key, out var val); return val; }
         ...
         private object SetValue(string key, object value, bool isAdd)
         {
            ...
            if (oldLength <= index)
            {
               ...
               Array.Resize(ref values, table.FieldNames.Length);
      ...
      public static Action<IDbCommand, object> CreateParamInfoGenerator(Identity identity, bool checkForDuplicates)
      {
         ...
         foreach (var prop in props)
         {
            ...
            if (checkForNull)
            {
               ...
               Label? allDone = dbType == DbType.String ? il.DefineLabel() : null;
...
#pragma warning restore 612, 618
   ...
   partial class FeatureSupport
   {
      ...
      public static FeatureSupport Get(IDbConnection connection)
      {
         ...
         return FeatureList.TryGetValue(name, out var features) ? features : FeatureList.Values.First();
      }
